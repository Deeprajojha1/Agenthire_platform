      ...
      {items.map(([href, label, Icon]) => {
        ...
        return (
          <Link
            ...
            href={href}
            onClick={onNavigate}
            className={`flex h-9 items-center gap-3 rounded-md px-3 text-sm font-medium transition ${active ? "bg-teal-700 text-white shadow-sm" : "text-slate-600 hover:bg-teal-50 hover:text-teal-800"}`}
          >
            <Icon size={15} /> {label}
          </Link>
        );
...
export default function AppShell({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f7]">
      ...
      <aside className="fixed inset-y-16 left-0 hidden w-60 border-r border-slate-200 bg-white md:block">
